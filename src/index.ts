...
export class SESClient {
  ...
  private flattenParams(obj: any, formData: URLSearchParams, prefix = ''): void {
      ...
      if (Array.isArray(obj[key])) {
        for (const [index, item] of obj[key].entries()) {
          if (typeof item === 'object') {
            this.flattenParams(item, formData, `${paramKey}.member.${index + 1}`)
          } else {
  ...
  private async parseError(responseText: string, statusCode: number): Promise<SESError> {
    try {
  ...
  async send(command: SendEmailCommand | SendTemplatedEmailCommand): Promise<SendEmailResponse> {
  ...
  private async sendEmail(input: SendEmailCommandInput): Promise<SendEmailResponse> {
  ...
  private async sendTemplatedEmail(input: SendTemplatedEmailCommandInput): Promise<SendEmailResponse> {
    const params: Record<string, any> = {
      Source: input.Source,
...
export async function sendEmail(client: SESClient, params: SendEmailCommandInput): Promise<SendEmailResponse> {
